...
import { createInsetShape, createOuterShape, shapeBounds } from './shapeOutline';
...
export function buildPlateGeometry(config: KeychainConfig): THREE.BufferGeometry {
  ...
  if (hole.enabled && hole.diameter > 0) {
    ...
    const cx = hole.offsetX;
    const cy = bounds.height / 2 - hole.offsetFromTop;
    holePath.absarc(cx, cy, hole.diameter / 2, 0, Math.PI * 2, true);
